...
def datapusher_submit(context, data_dict):
    ...
    # MUST BE A JWT TOKEN GENERATED WITH THE USER DEFAULT (WITH THE SAME NAME OR SITE_ID)
    ckan_datapusher_token = config.get_value('ckan.datapusher.token')
    if  ckan_datapusher_token is None:
        invalid_token_error = "No ckan.datapusher.token configured in ckan ini file. Please insert a valid token generated by an admin user for the user %s " % site_user['id']
    ...
    try:
        ...
        r = requests.post(
            ...
            data=json.dumps({
                # MODIFIED FOR NEW JWT TOKEN
                #'api_key': site_user['apikey'],
                'api_key': ckan_datapusher_token,
